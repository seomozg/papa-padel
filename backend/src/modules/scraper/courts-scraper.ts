...
// Поиск через Google Places API (Text Search)
async function searchCourtsGoogle(query: string): Promise<any[]> {
  try {
...
function extractCity(address: string): string {
  ...
  for (const part of parts) {
    ...
    if (trimmed.includes('Москва') || trimmed.includes('Moscow')) return 'Москва';
    if (trimmed.includes('Санкт-Петербург') || trimmed.includes('Saint Petersburg')) return 'Санкт-Петербург';
    if (trimmed.includes('Новосибирск')) return 'Новосибирск';
    if (trimmed.includes('Екатеринбург')) return 'Екатеринбург';
    if (trimmed.includes('Казань')) return 'Казань';
    if (trimmed.includes('Нижний Новгород')) return 'Нижний Новгород';
    if (trimmed.includes('Челябинск')) return 'Челябинск';
    if (trimmed.includes('Самара')) return 'Самара';
    if (trimmed.includes('Ростов')) return 'Ростов-на-Дону';
    if (trimmed.includes('Уфа')) return 'Уфа';
    if (trimmed.includes('Красноярск')) return 'Красноярск';
    if (trimmed.includes('Воронеж')) return 'Воронеж';
    if (trimmed.includes('Краснодар')) return 'Краснодар';
    if (trimmed.includes('Сочи')) return 'Сочи';
    if (trimmed.includes('Калининград')) return 'Калининград';
  }
...
export async function scrapeAllCourts() {
  console.log('🎾 Начинаем парсинг падел-кортов через Google Places API...\n');
  ...
  for (const query of SEARCH_QUERIES) {
    ...
    const places = await searchCourtsGoogle(query);
    ...
    for (const place of places) {
      ...
      // Часы работы
      const workingHours = details?.opening_hours?.weekday_text?.join(', ') || null;

      // Тип корта
      const type = determineCourtType(name, types);
